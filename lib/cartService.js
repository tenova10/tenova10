...
export async function validateCart(cart, { strict = false } = {}) {
  ...
  for (const item of cart) {
    ...
    if (existing) {
      ...
      itemById.set(item.id, normalizedItem)
  ...
  for (const item of normalizedInput) {
    ...
    if (availableStock <= 0) {
      messages.push(`"${product.name}" is out of stock and was removed from your cart.`)
      continue
    }
    ...
    if (qty > availableStock) {
      messages.push(
        `Quantity of "${product.name}" was reduced from ${qty} to ${availableStock} because only ${availableStock} remain available.`
      )
      qty = availableStock
    }
